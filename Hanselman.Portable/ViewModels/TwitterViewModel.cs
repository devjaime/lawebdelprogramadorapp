    ...
    public class TwitterViewModel : BaseViewModel
    {
        ...
        public async Task ExecuteLoadTweetsCommand()
        {
                  ...
                  (from tweet in queryResponse
                   select new Tweet
                   {
                       ...
                       Image = tweet.RetweetedStatus != null && tweet.RetweetedStatus.User != null ?
                                    tweet.RetweetedStatus.User.ProfileImageUrl.Replace("http://", "https://") : (tweet.User.ScreenNameResponse == "HsJhernandez" ? "scott159.png" : tweet.User.ProfileImageUrl.Replace("http://", "https://"))
                   }).ToList();
            ...
            if (error)
            {
                ...
                await page.DisplayAlert("Error", "En este momento no se pudieron cargar los tweets.", "OK");
            }
